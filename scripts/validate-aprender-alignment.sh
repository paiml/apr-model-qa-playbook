...
get_size_from_model_id() {
    ...
    local size_match
    size_match=$(echo "${name}" | grep -oE '[0-9]+\.?[0-9]*[Bb]' | head -1)
    if [ -n "${size_match}" ]; then
        echo "${size_match}" | sed 's/[Bb]$/b/' | tr '[:upper:]' '[:lower:]'
    else
        echo "unknown"
    fi
}
...
main() {
    ...
    if [ ! -f "${models_csv}" ]; then
        ...
        local line_num=0
        while IFS=',' read -r model_id _family _parameters size_category rest; do
            ((line_num++)) || true
