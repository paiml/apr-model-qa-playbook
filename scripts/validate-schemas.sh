...
validate_models_csv() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        # Parse CSV fields
        IFS=',' read -r model_id family parameters size_category status mqs_score grade certified_tier last_certified g1 g2 g3 g4 rest <<< "$line"
        ...
        if [[ ! " $valid_sizes " =~ " $size_category " ]]; then
        ...
        if [[ ! "$mqs_score" =~ ^[0-9]+$ ]] || [[ "$mqs_score" -lt 0 ]] || [[ "$mqs_score" -gt 1000 ]]; then
        ...
        if [[ ! "$last_certified" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T ]]; then
